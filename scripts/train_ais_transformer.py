...
def load_tracks(
    ...
    Returns (tracks, vessel_types) where:
      tracks        : mmsi -> float32 (T, 3) = [lon, lat, dt_seconds]
      vessel_types  : mmsi -> int vessel type code (0 = unknown/missing)
    """
    ...
    for v_id, g in df.groupby(id_col):
        ...
        if pts.shape[0] >= 5:
            key = str(v_id)
            tracks[key] = pts
            # Vessel type: take first non-null value for this vessel
...
def train_val_split(tracks, vessel_types, val_frac, seed):
...
class CausalDataset(Dataset):
    ...
    y         : (seq_len, 2)           targets = displacement to next pos at each step
    gap_mask  : (seq_len,) bool        True where dt > max_gap_sec
    ...
    def __init__(self, tracks, vessel_types, vtype_vocab, scalers, seq_len,
                 stride=1, max_windows_per_track=None, use_velocity=False,
                 max_gap_sec=600):
        self.tracks       = tracks
    ...
    def __getitem__(self, idx):
        ...
        y = self.scalers.disp.transform(disp_raw)                    # normalized displacement
...
class SingleStepDataset(Dataset):
    ...
    x         : (seq_len, input_dim)  full context window
    y         : (2,)                  displacement for the step after the window
    ...
    def __getitem__(self, idx):
        ...
        x = make_input(pts[:-1], self.scalers, self.use_velocity)  # (seq_len, D)
        disp_raw = pts[-1:, :2] - pts[-2:-1, :2]                    # (1, 2) last step delta
        y = self.scalers.disp.transform(disp_raw)[0]                 # (2,) normalized displacement
...
def get_loader(tracks, vessel_types, vtype_vocab, scalers, seq_len, batch_size,
               pred_mode="causal", stride=1, max_windows_per_track=None,
    ...
    if pred_mode == "causal":
        dataset  = CausalDataset(tracks, vessel_types, vtype_vocab, scalers,
                                 ...
                                 use_velocity, max_gap_sec=max_gap_sec)
...
class AISTransformer(nn.Module):
    """
    Transformer encoder for next-step displacement prediction.

    pred_mode="causal":
        Causal (triangular) attention mask. Output at position t predicts
        displacement to t+1. At inference, read off the last output.

    pred_mode="single":
        No mask. Full bidirectional context. Mean pool → single prediction.

    Vessel type conditioning: if num_vessel_types > 0, a learned embedding
    for each AIS vessel type code is added to the model's hidden state after
    input projection (same type broadcast across all timesteps, like BERT
    segment embeddings).
    """
    ...
    def __init__(self, input_dim, d_model=128, nhead=8,
                 ...
                 pred_mode="causal",
                 num_vessel_types=0, vessel_type_embed_dim=8):
        super().__init__()
        ...
        # Optional vessel type conditioning
    ...
    def forward(self, x, gap_mask=None, vessel_type=None):
        """
        Args:
        ...
        # Vessel type conditioning: add per-vessel bias to all timesteps
        if hasattr(self, "vessel_type_emb") and vessel_type is not None:
            ...
            x = x + type_emb.unsqueeze(0)           # broadcast: (seq_len, B, d_model)
...
def evaluate(model, loader, device, scalers, pred_mode):
    ...
    for x, y, gap_mask, vtype in loader:
        ...
        # Input positions (normalized) → geographic, used to reconstruct next pos
        x_np = x.cpu().numpy()[:, :, :2]   # (seq_len, B, 2) or (seq_len, B, 2)

        if pred_mode == "causal":
            # pred: (seq_len, B, 2) normalized displacement
            # y:    (seq_len, B, 2) normalized displacement
            ...
            # pred: (B, 2) normalized displacement, x_np[-1]: last input position
            last_pos_geo = scalers.pos.inverse(x_np[-1])             # (B, 2)
            pred_disp_geo = scalers.disp.inverse(pred.cpu().numpy()) # (B, 2)
            true_disp_geo = scalers.disp.inverse(y.cpu().numpy())    # (B, 2)
...
def evaluate_constant_velocity_baseline(loader, device, scalers, pred_mode):
    ...
    # Scale factor to convert pos-normalized diffs → disp-normalized space
    scale = torch.tensor(scalers.pos.std / scalers.disp.std, dtype=torch.float32)
    ...
    for x, y, _gap, _vtype in loader:
        ...
        if pred_mode == "causal":
            # vel in pos-normalized space; convert to disp-normalized for fair MSE + ADE
            vel_pos_diff = torch.diff(pos, dim=0, prepend=pos[:1])  # (seq_len, B, 2)
            vel_norm = vel_pos_diff * scale.view(1, 1, 2).to(device)  # disp-normalized
            ...
            # Single mode: displacement from last two input positions, converted to disp-normalized
            vel_norm = (pos[-1] - pos[-2]) * scale.view(1, 2).to(device)  # (B, 2)
...
def sanity_check(model, loader, device, scalers, pred_mode):
    ...
    # Last input position (used to reconstruct predicted next position)
    if pred_mode == "causal":
    ...
    for i in range(min(3, p_batch.shape[0])):
        ...
        p = inp_pos + p_disp  # predicted next position
        t = inp_pos + t_disp  # true next position
...
def main():
    ...
    parser.add_argument("--lambda_smooth",  type=float, default=0.1,
                        help="Weight for speed-smoothness regularisation "
                             "(supervisor suggestion: MSE + lambda*accel^2). 0=off.")
    ...
    num_vessel_types = len(vtype_vocab)  # e.g. 28 (27 codes + 0=unknown)
