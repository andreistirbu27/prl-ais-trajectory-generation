#!/usr/bin/env python3
"""
visualize.py — AIS data distribution, raw trajectory samples, and model predictions.

Produces:
  viz_distribution.png  — train/val vessel coverage on the US coast
  viz_tracks.png        — raw trajectories (--n_train train + --n_val val) coloured by time
  viz_predictions.png   — model vs constant-velocity vs truth for val vessels
                          (requires --checkpoint)

Usage:
    # Distribution + track samples only:
...
# ── Figure 2: raw trajectory samples ─────────────────────────────────────────

def approx_dist_km(pts):
    lat_mid = pts[:, 1].mean()
    dlon = np.diff(pts[:, 0]) * 111.0 * math.cos(math.radians(lat_mid))
    dlat = np.diff(pts[:, 1]) * 111.0
    return float(np.sqrt(dlon**2 + dlat**2).sum())


def _draw_track_panel(ax, pts, vid, vtype_code, split_label, cmap):
    lons, lats = pts[:, 0], pts[:, 1]
    n_seg = len(lons) - 1
    for j in range(n_seg):
        t = j / max(n_seg - 1, 1)
        color = cmap(0.25 + 0.75 * t)
        ax.plot(lons[j:j+2], lats[j:j+2], color=color, linewidth=0.8, alpha=0.7 + 0.3 * t)

    ax.scatter(lons[0],  lats[0],  marker="s", color="#2e7d32", s=50, zorder=6)
    ax.scatter(lons[-1], lats[-1], marker="o", color="#c62828", s=50, zorder=6)

    dist_km = approx_dist_km(pts)
    vname = VTYPE_NAME.get(vtype_code, f"Type {vtype_code}")
    ax.set_title(f"{vid}  [{vname}]", fontsize=7, pad=2)
    ax.text(0.02, 0.02,
            f"{len(lons)} pts | ~{dist_km:.0f} km | {split_label}",
            transform=ax.transAxes, fontsize=6,
            color="white", bbox=dict(fc="#000000aa", boxstyle="round,pad=0.2"))
...
def plot_tracks(train_tracks, train_vtypes, val_tracks, val_vtypes,
                n_train, n_val, out_path, seed=42):
    rng = random.Random(seed)

    def sample(track_dict, k):
        keys = list(track_dict.keys())
        rng.shuffle(keys)
        return keys[:k]

    train_ids = sample(train_tracks, n_train)
    val_ids   = sample(val_tracks,   n_val)

    panels = [(vid, train_tracks[vid], train_vtypes.get(vid, 0), "TRAIN") for vid in train_ids]
    panels += [(vid, val_tracks[vid],  val_vtypes.get(vid, 0),  "VAL")   for vid in val_ids]
    ...
    fig.suptitle(
        "Raw vessel trajectories — blue panels = train, red panels = val\n"
        "■ = track start   ● = track end   colour darkens toward present",
        fontsize=11, y=1.01,
    )
    plt.tight_layout()
    plt.savefig(out_path, dpi=150, bbox_inches="tight")
...
# ── Figure 3: model prediction panels ────────────────────────────────────────
...
def plot_predictions(model, val_tracks, val_vtypes, vtype_vocab, scalers,
    ...
    for i, (vid, pts) in enumerate(chosen):
        ...
        window = pts[start : start + seq_len + 1]
